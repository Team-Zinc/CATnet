...
void SessionEncryptionState::set_other_private_key(unsigned char oth_pk[crypto_box_PUBLICKEYBYTES]) {
    memcpy(m_oth_pk, oth_pk, crypto_box_PUBLICKEYBYTES);
}

std::vector<unsigned char> SessionEncryptionState::encrypt_message(std::vector<unsigned char> message) {
    ...
    unsigned char *ciphertext = (unsigned char *) sodium_malloc(crypto_box_MACBYTES + message.size());
    if (ciphertext == nullptr) {
        throw std::system_error(errno, std::generic_category());
    }

    if (crypto_box_easy(ciphertext, message.data(), message.size(), nonce, m_our_pk, m_our_sk) == -1) {
